...
static inline int loadmem(uintptr_t addr) {
  int tmp;
#ifdef NOELV_SYSTEST
...
#define FIFO_RX_CNT ((loadmem((uintptr_t)&uart->status) >> 26) & 0x3F)
#define FIFO_TX_CNT ((loadmem((uintptr_t)&uart->status) >> 20) & 0x3F)
#define FIFO_TX_EMPTY ((loadmem((uintptr_t)&uart->status) >> 2) & 0x1)
#define FIFO_TX_FULL (loadmem((uintptr_t)&uart->status) & 0x200)
#define FIFO_RX_FULL (loadmem((uintptr_t)&uart->status) & 0x400)
#define FIFO_RX_HALF_FULL (loadmem((uintptr_t)&uart->status) & 0x100)
#define DATA_READY (loadmem((uintptr_t)&uart->status) & 1)
#define SHIFTREG_TX_EMPTY (loadmem((uintptr_t)&uart->status) & 2)
...
int apbuart_test(uintptr_t addr) {
  ...
  temp = loadmem((uintptr_t)&uart->status);
  while ((temp & 1) || !(temp & 4) || !(temp & 2) ) {
    while ((temp & 1) || !(temp & 4) || !(temp & 2) ) {
      temp = loadmem((uintptr_t)&uart->data);
      temp = loadmem((uintptr_t)&uart->status);
    }
    temp = loadmem((uintptr_t)&uart->status);
  }
  ...
  if (fifosize > 1) {
    // if (((loadmem((uintptr_t)&uart->status) & 0x80) != 0) ) {
    //   fail(4); // th bit incorrect
    //   return 4;
    // }
  }
  uart->data = (uintptr_t) test[0];
	...
	// if ((loadmem((uintptr_t)&uart->status) & 4) == 4) {
  ...
  if (fifosize > 1) {
    for (i = 1; i < fifosize; i++) {
      uart->data = (uintptr_t) test[i % testsize];
    }
		// NOTE: This test is temporarily removed
		// if (((loadmem((uintptr_t)&uart->status) & 0x80) == 0x80) ) {
		//   /*th bit incorrect*/
		//   fail(5);
    //   return 5;
		// }
