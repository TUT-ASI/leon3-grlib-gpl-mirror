...
struct i2cmstregs {
  volatile unsigned int prer; /* Prescaler Register */
  volatile unsigned int ctr; /* Control Register */
  volatile unsigned int xr; /* Transmit (wr only) and Receive (rd only) Register */
  volatile unsigned int csr; /* Command (wr only) and Status (rd only) Register */
};
 ...
 *
 */
int i2cmst_test(uintptr_t addr) {
  int i;
