...
#error "Please define a way to initialize a memory block."
...
#if HAS_FLOAT
	ee_printf("Total time (secs): %ld\n", time_secs);
	if (time_in_secs(total_time) > 0)
		ee_printf("Iterations/Sec   : %ld\n",default_num_contexts*results[0].iterations/time_secs);
#else 
...
#if HAS_FLOAT
		if (known_id==3) {
			ee_printf("CoreMark 1.0 : %ld / %s %s",default_num_contexts*results[0].iterations/time_secs,COMPILER_VERSION,COMPILER_FLAGS);
	...
	return total_errors; // If returned value is different than 0, then TB will signalize error;
//	return MAIN_RETURN_VAL;	
}
