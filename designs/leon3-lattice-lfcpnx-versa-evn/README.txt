This design is tailored to the Lattice CertusPRO-NX VERSA Evaluation board
---------------------------------------------------------------------

= Introduction =

This directory contains a template design for the CertusPRO-NX VERSA Evaluation
board (EVB). The board features DDR3 memory and quite some peripherals.
...
LATTICE_IP
	list the Lattice IPs that you want to get included in the design.  The
	corresponding cfg files need to be present inside the folder
	boards/lattice-lfcpnx-versa-evn. Each IP needs to be listed as path to
	...
	it. Currently the only IP listed is pll_125i_62_5o, which is
	configured to take the 125MHz clock signal from an external oscillator
	and output the clock signal for the design.

Design clock frequency: 50 MHz
By reducing the AHBRAM to 64KB, the user can increase the clock frequency to
62.5 MHz. With 50MHz however, the user have more freedom to experiment,
e.g. by instantiating an FPU.

To trigger a reset of the Leon3 processor, the user can press button SW3
