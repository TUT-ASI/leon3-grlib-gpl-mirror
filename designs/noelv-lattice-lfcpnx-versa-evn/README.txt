RISC-V NOEL-V Template Design for Lattice CertusPro-NX VERSA Evaluation board
----------------------

= Introduction =

This directory contains a template design for the CertusPRO-NX VERSA Evaluation
board (EVB), which features a LPDDR4 memory.
...
LATTICE_IP
	...
	it. Currently the only IP listed is pll_12MHz, which is configured
	to take the 12MHz clock signal from an external oscillator and output
	the clock signal for the design.

Currently, only the NoelV core with 128KB on-chip ram are instantiated.
To change the size of the on-chip ram, the user needs to manually edit the
parameter for it inside rtl/noelvmp.vhd
